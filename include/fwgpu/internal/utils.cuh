...
namespace internal {
  ...
  inline auto alloc_and_init_device_gemm_mats(
    ...
    return std::make_tuple(
        reinterpret_cast<T *>(d_A), reinterpret_cast<T *>(d_B),
        reinterpret_cast<T *>(d_C));
  }
  ...
  inline auto alloc_and_init_device_gemm_mats(
    ...
    return std::make_tuple(
        reinterpret_cast<T *>(d_A), reinterpret_cast<T *>(d_B),
        reinterpret_cast<T *>(d_C), reinterpret_cast<T *>(d_D));
  }

  template <typename T>
  inline auto dealloc_device_gemm_mats(std::tuple<T *, T *, T *, T *> device_ptrs)
      -> void {
    cudaFree(std::get<0>(device_ptrs));
    cudaFree(std::get<1>(device_ptrs));
    cudaFree(std::get<2>(device_ptrs));
